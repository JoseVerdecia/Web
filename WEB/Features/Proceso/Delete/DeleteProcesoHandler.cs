...
public class DeleteProcesoHandler : IRequestHandler<DeleteProcesoRequest, Unit>
{
    ...
    public async Task<AppResult<Unit>> Handle(DeleteProcesoRequest request, CancellationToken cancellationToken)
    {
        ...
        if (proceso.IsDeleted && !request.Permanent)
            return AppResult<Unit>.Fail("El proceso ya está eliminado.");
        ...
        if (!string.IsNullOrEmpty(proceso.JefeProcesoId))
        {
            ...
            if (resetResult.IsFailure)
                return AppResult<Unit>.Fail($"No se pudo resetear el rol del jefe de proceso: {resetResult.Errors}");
        }
