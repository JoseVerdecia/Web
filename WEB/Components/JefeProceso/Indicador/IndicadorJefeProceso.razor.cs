...
namespace WEB.Components.JefeProceso.Indicador;

public partial class IndicadorJefeProceso : ComponentBase
{
    ...
    private IndicadorDisplayItem? selectedIndicador; // <-- NUEVO
    ...
    private async Task OpenEditDialog(IndicadorDisplayItem displayItem)
    {
        ...
        var updateRequest = new UpdateIndicadorRequest
        {
            ...
            ProcesoId = indicador.Proceso?.Id ?? 0,
            ObjetivoIds = indicador.Objetivos?.Select(o => o.Id).ToList(),
            MetaCumplirPorArea = metasPorArea // <-- ACTUALIZADO
        };
        ...
        {
            var index = indicadores.FindIndex(i => i.Id == updatedIndicador.Id);
            if (index >= 0)
            {
                indicadores[index] = updatedIndicador;
                
                var newItem = IndicadorDisplayItem.FromIndicadorDto(updatedIndicador);
                gridItems = indicadores.Select(IndicadorDisplayItem.FromIndicadorDto).ToList();

                // <-- NUEVO: Actualizar panel de detalle si es el indicador seleccionado
