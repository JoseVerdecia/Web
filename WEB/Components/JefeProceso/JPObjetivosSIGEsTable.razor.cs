...
public partial class JPObjetivosSIGEsTable : ComponentBase
{
    ...
    private async Task CargarDatos()
    {
        ...
        try
        {
            var result = await Mediator.Send(new GetAllIndicadoresByObjetivoAndProcesoRequest(ObjetivoId, ProcesoId));
            if (result.IsSuccess && result.Value != null)
            {
                ...
                foreach (var ind in indicadores)
                {
                    ...
                    {
                        Numero = secuencia.ToString(),
                        EsPrimeraDelGrupo = !procesoCellWritten,
                        RowspanProceso = total,
